...
NTSTATUS driver_main(PDRIVER_OBJECT driver_object, PUNICODE_STRING registry_path) {
	...
	RtlInitUnicodeString(&device_name, L"\\Device\\Droiver");
	...
	RtlInitUnicodeString(&symbolic_link, L"\\DosDevices\\Droiver");

	status = IoCreateSymbolicLink(&symbolic_link, &device_name);
	if (status != STATUS_SUCCESS) {
		debug_print("[-] Failed to establissh symbolic_link\n");
		return status;
	}
...
NTSTATUS DriverEntry() {
	debug_print("[+] Fick deine Oma\n");

	UNICODE_STRING driver_name = {};
	RtlInitUnicodeString(&driver_name, L"\\Driver\\Droiver");
